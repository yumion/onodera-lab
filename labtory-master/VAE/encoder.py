...
def load_img_to_numpy(dirname,suffix):
    ...
    numpy_images = []
    filenames = sorted(glob.glob(dirname+suffix))
    ...
    numpy_images /= 255
    return filenames,numpy_images

#x_train = load_img_to_numpy('./cropped_images/','*.tif')
filenames,x_test = load_img_to_numpy('./cropped_test/','*.tif')
...
vae.load_weights('colon_cancer_encoder.h5')
...
with open('plot_data.csv','w') as fw:
